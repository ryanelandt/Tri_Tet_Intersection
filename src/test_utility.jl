...
normal(plane::SMatrix{1,4,Float64,4}) = SVector{3,Float64}(plane[1], plane[2], plane[3])
dist_from_plane(plane::SMatrix{1,4,Float64,4}, v::SVector{3,Float64}) = dot(v, normal(plane)) + plane[4]
project_into_plane(plane::SMatrix{1,4,Float64,4}, v::SVector{3,Float64}) = v - dist_from_plane(plane, v) * normal(plane)
